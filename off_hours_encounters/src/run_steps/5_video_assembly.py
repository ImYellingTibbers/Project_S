...
# Intro bass SFX
# -------------------------
INTRO_SFX_REL = os.getenv("RENDER_INTRO_SFX", "src/assets/sfx/spooky_bass_start.mp3")
INTRO_SFX_GAIN_DB = float(os.getenv("RENDER_INTRO_SFX_GAIN_DB", "-10.0"))  # additional trim after loudnorm
INTRO_SFX_I_LUFS = float(os.getenv("RENDER_INTRO_SFX_I_LUFS", "-28.0"))     # cap loudness (integrated)
INTRO_SILENCE_GAP_S = float(os.getenv("RENDER_INTRO_SILENCE_GAP_S", "0.20"))  # 150–250ms recommended
INTRO_FADEIN_S = float(os.getenv("RENDER_INTRO_FADEIN_S", "0.12"))            # quick video fade-in
INTRO_VIS_RAMP_S = float(os.getenv("RENDER_INTRO_VIS_RAMP_S", "0.80"))        # first-second intensity ramp
...
def _build_video_filter_complex(audio_dur: float, stitched_dur: float) -> str:
    ...
    horror_grade = (
        "[vbase]"
        "hue=s=0.78,"
        "eq=contrast=1.22:brightness=-0.06:gamma=0.94,"
        "colorbalance=bs=0.06:gs=0.02"
        "[vgraded]"
    )
...
def _render_segment_clip(tmp_dir: Path, image_path: Path, seg_idx: int, duration: float, seed: str) -> Path:
    ...
    intro_ramp = is_first and INTRO_VIS_RAMP_S > 0
    intro_fadein = is_first and INTRO_FADEIN_S > 0

    # First-frame interrupt (glitch splice)
    if is_first and ENABLE_FIRST_FRAME_INTERRUPT:
        glitch = (
            f"[0:v]"
            f"scale={TARGET_W}:{TARGET_H},"
            f"eq=contrast=1.6:brightness=-0.20,"
    ...
    # Build post-processing chain (ONLY for first segment)
    post = []
    if intro_ramp:
        # Contrast / brightness ramp over first ~0.8s
        post.append(
            "eq="
            f"contrast='1.0+0.25*if(lt(t,{INTRO_VIS_RAMP_S}),t/{INTRO_VIS_RAMP_S},1)':"
            f"brightness='-0.02+0.06*if(lt(t,{INTRO_VIS_RAMP_S}),t/{INTRO_VIS_RAMP_S},1)'"
        )
        # Subtle chromatic unease (only during ramp)
        post.append(
            f"rgbashift=rh=0.6:bh=-0.6:enable='lt(t,{INTRO_VIS_RAMP_S})'"
        )

    if intro_fadein:
        post.append(f"fade=t=in:st=0:d={INTRO_FADEIN_S}")

    post_chain = ("," + ",".join(post)) if post else ""

    # Final filter_complex
    if is_first and ENABLE_FIRST_FRAME_INTERRUPT:
        filter_complex = (
            f"{vf_chain};"
            f"[v0]{post_chain.lstrip(',')},"
            f"trim=duration={duration:.6f},setpts=PTS-STARTPTS[v]"
        )
    else:
        filter_complex = (
            f"{vf_chain}{post_chain},trim=duration={duration:.6f},setpts=PTS-STARTPTS[v]"
        )
...
def main() -> int:
    ...
    # Intro SFX path (project-relative)
    intro_sfx_path = (PROJECT_ROOT / INTRO_SFX_REL).resolve()
    intro_sfx_enabled = intro_sfx_path.exists()
    if intro_sfx_enabled:
        print(f"[audio] intro sfx -> {intro_sfx_path}")
    else:
        print(f"[audio] intro sfx not found (skipping): {intro_sfx_path}")

    # Target output duration accounts for intro silence gap
    target_audio_dur = audio_dur
    if intro_sfx_enabled and INTRO_SILENCE_GAP_S > 0:
        target_audio_dur = audio_dur + INTRO_SILENCE_GAP_S
    ...
    for s in segments:
        ...
        if seg_idx == 0 and intro_sfx_enabled and INTRO_SILENCE_GAP_S > 0:
            dur += INTRO_SILENCE_GAP_S
    ...
    if MUSIC_ENABLED:
        try:
            ...
            bed_path = _build_bed_audio(tmp_dir, bed_src, target_audio_dur)
    ...
    video_fc = _build_video_filter_complex(target_audio_dur, stitched_dur)

    if bed_path and bed_path.exists():
        gap_ms = int(max(0.0, INTRO_SILENCE_GAP_S) * 1000.0)

        vo_chain = (
            f"[1:a]aresample=48000,"
            f"{'adelay=' + str(gap_ms) + '|' + str(gap_ms) + ',' if (intro_sfx_enabled and gap_ms > 0) else ''}"
            f"afade=t=out:st={max(target_audio_dur-0.4, 0):.2f}:d=0.35,"
            f"alimiter=limit=0.98[vo]"
        )

        bed_chain = "[2:a]aresample=48000,volume=1.0[bed]"
        ...
        if intro_sfx_enabled:
            sfx_chain = (
                f"[3:a]aresample=48000,"
                f"loudnorm=I={INTRO_SFX_I_LUFS:.1f}:TP=-2:LRA=7,"
                f"volume='{INTRO_SFX_GAIN_DB:.2f} + 3*min(t/0.25,1)',"
                f"alimiter=limit=0.98[sfx]"
            )
            mix_inputs = "[vo][bed][sfx]"
            mix_n = 3

        audio_fc = (
            vo_chain + ";" +
            bed_chain +
            ((";" + sfx_chain) if intro_sfx_enabled else "") + ";" +
            f"{mix_inputs}amix=inputs={mix_n}:duration=longest:dropout_transition=0,"
            f"atrim=duration={target_audio_dur:.6f},"
        ...
        _run([
            ...
            "-i", _ffmpeg_path(audio_path),      # 1:a (VO)
            "-i", _ffmpeg_path(bed_path),        # 2:a (music bed)
            *(
                ["-i", str(intro_sfx_path)]      # 3:a (intro bass SFX)
                if intro_sfx_enabled else []
            ),
            "-filter_complex", fc,
            "-map", "[vout]",
            "-map", "[aout]",
        ...
        _run([
            ...
            "-i", _ffmpeg_path(audio_path),     # 1:a (VO)
            *(
                ["-i", str(intro_sfx_path)]     # 2:a (intro bass SFX)
                if intro_sfx_enabled else []
            ),
            "-filter_complex", fc,
            "-map", "[vout]",
